...
static cvarTable_t		gameCvarTable[] = {
	// noset vars (no vmCvar_t variable)
...
void G_RemapTeamShaders( void ) {
	...
	// determined by current g_redteam/g_blueteam values -- never
	// inherited from a prior team setting. For real clan names we point
	// textures/ctf2/redteam01 at team_icon/<clan>_red. For color-word
	// teams ("Red"/"Blue") the derived shader name (team_icon/Red_red)
	// has no artwork, so we remap-to-self instead -- the engine treats
	// sh == sh2 as "clear the remap" (see RE_RemapShader, tr_shader.c),
	// which lets our trinity_teamicons.shader fallback take effect
	// without spamming "new shader not found" warnings or requiring
	// a vid_restart to purge a stale clan remap.
...
void LogExit( const char *string ) {
	...
	for (i=0 ; i < numSorted ; i++) {
		...
		// the reset the in-game console (G_LogPrintf fans out to
		// G_Printf when g_dedicated) inherits that color for the
		// next line's timestamp -- most visibly during the /rcon
		// rotate scoreboard dump.
		G_LogPrintf( "score: %i  ping: %i  client: %i %s" S_COLOR_WHITE "\n", cl->ps.persistant[PERS_SCORE], ping, level.sortedClients[i],	cl->pers.netname );
#ifdef MISSIONPACK
	...
	if ( g_timelimit.integer && !level.warmupTime ) {
		if ( level.time - level.startTime >= g_timelimit.integer*60000 ) {
			...
			if ( g_gametype.integer == GT_FFA || g_gametype.integer == GT_TOURNAMENT ) {
				...
				// ScoreIsTied() above redirected ties into overtime, so a
				// strict winner exists here.
