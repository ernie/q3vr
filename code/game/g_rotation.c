...
qboolean ParseMapRotation( void )
{
	...
	{
		G_Printf( S_COLOR_YELLOW "%s: map rotation file doesn't exists.\n", g_rotation.string );
		return qfalse;
	}
	if ( len >= sizeof( buf ) )
	{
		G_Printf( S_COLOR_YELLOW "%s: map rotation file is too big.\n", g_rotation.string );
		len = sizeof( buf ) - 1;
	}
...
__rescan:
	...
	{
		G_Printf( S_COLOR_YELLOW "%s: no maps in rotation file.\n", g_rotation.string );
	...
	if ( !map[0] ) // map at required index not found?
	{
		...
		{
			G_Printf( S_COLOR_CYAN "%s: map at index %i not found, rescan\n", g_rotation.string, reqIndex );
			reqIndex = 1;
			goto __rescan;
		}
		trap_Cvar_Set( SV_ROTATION, "1" );
