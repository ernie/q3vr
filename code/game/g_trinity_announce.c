...
// handshake-challenge pattern (challenge generated in ClientBegin,
// sent from G_RunFrame).
...
void G_TrinityProcessAnnouncements( void ) {
	...
	// Mirrors the gate in G_CheckMinimumPlayers (g_bot.c), which is why
	// announcements driven by bot_minplayers already work -- those bots
	// don't get added until after the same 2s mark.
	...
	for ( pass = 0; pass < 2; pass++ ) {
		...
		for ( i = 0; i < level.maxclients; i++ ) {
			...
			trap_SendServerCommand( -1, va("tann j %d", i) );
...
void G_TrinityAnnounceWinner( int clientNum ) {
	...
	trap_SendServerCommand( -1, va("tann w %d", clientNum) );
}
