...
Write data through zstd streaming compressor to file.
===============
*/
...
Generate a default recording name, preferring g_matchUUID if available.
===============
*/
...
static void SV_TV_StartRecord( const char *filename ) {
	...
	// Initialize zstd streaming compressor (level -3 = very fast, suitable for real-time)
...
void SV_TV_WriteFrame( void ) {
	...
	// Auto-record threshold: check if enough non-spectator human players are present
...
void SV_TV_StopRecord( qboolean discard ) {
	...
	if ( discard ) {
		...
		// Free compressor and close file without finalizing
		if ( tv.cstream ) {
		...
		// Write uncompressed k/v trailer for duration metadata
		{
			...
			SV_TV_FileWrite( "", 1, tv.file );  // empty key = terminator

			trailerSize = (int)( tv.fileOffset - trailerStart + 4 );  // +4 for this field itself
			SV_TV_FileWrite( &trailerSize, 4, tv.file );
		}
