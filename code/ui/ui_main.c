...
================
*/
// ui cvars are declared via ui_cvar.h include below
...
static void UI_DrawTrinitySigil( rectDef_t *rect ) {
	...
	// Compensate for the renderer's NOWORLDMODEL cropFactor so the
	// effective projection matches the desired 30x30 FOV.
	{
		float cropHeight = uiInfo.uiDC.glconfig.vidWidth * 0.75f;
		float cropFactor = uiInfo.uiDC.glconfig.vidHeight / cropHeight;
		refdef.fov_x = 2.0f * RAD2DEG( atan( tan( DEG2RAD( 30.0f ) * 0.5f ) / cropFactor ) );
		refdef.fov_y = refdef.fov_x;
	}

	trap_R_ModelBounds( trinityModel, mins, maxs );
	len = 0.5 * ( maxs[2] - mins[2] );
	origin[0] = len / tan( DEG2RAD( 30.0f ) * 0.5 );
...
static void UI_RunMenuScript(const char **args) {
	...
	if (String_Parse(args, &command)) {
		...
		} else if (Q_stricmp(command, "updateDownload") == 0) {
			trap_Cmd_ExecuteText( EXEC_APPEND, "updatedownload\n" );
		} else if (Q_stricmp(command, "updateCancel") == 0) {
			trap_Cmd_ExecuteText( EXEC_APPEND, "updatecancel\n" );
		} else if (Q_stricmp(command, "updateRestart") == 0) {
...
// declared but not in cvar macro (not in cvarTable, or no vmCvar_t)
