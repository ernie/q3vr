 ...
 *
 * renderergl2 (rend2) variant: world surfaces are unified srfBspSurface_t with
 * indexes[]/verts[] (srfVert_t); R_AddDrawSurf takes the 6-arg rend2 form.
 */
#include "tr_local.h"
...
static void R_DecalClipTriangle( const vec3_t origin, float radius, vec3_t axis[3],
	...
	for ( k = 0; k < numPoly; k++ ) {
		...
		v[k].modulate[0] = color[0];
		v[k].modulate[1] = color[1];
		v[k].modulate[2] = color[2];
		v[k].modulate[3] = color[3];
	}
	...
	for ( i = 0; i < MAX_DECAL_FRAGMENTS; i++ ) {
		...
		if ( d->lifeTime > 0 ) {
			...
			for ( j = 0; j < d->srf.numVerts; j++ ) {
				d->srf.verts[j].modulate[3] = (byte)( ( d->baseColor[3] * fade ) / 255 );
			}
		}
