...
static void CG_DrawTVOverlay( void ) {
	...
	// Download progress is informational, not a yes/no prompt.
	if ( cg_tvdOffer.string[0] && cg.tvdOfferName[0] ) {
		hasDialog = qtrue;
	}
	if ( cgs.voteTime && ( cg.time - cgs.voteTime < VOTE_TIME ) ) {
		hasDialog = qtrue;
	}
	{
		int cs_offset = -1;
