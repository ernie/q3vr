...
static void CG_ParseScores( void ) {
	...
	// Playback: the "scores" command carries stale team scores; read the
	// authoritative CS_SCORES directly.
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "trinity_challenge" ) ) {
		...
		// Trinity version (read from pk3 at runtime)
		{
			fileHandle_t f;
			int len;
			hs_version[0] = '\0';
			...
			if ( len > 0 ) {
				if ( len >= (int)sizeof( hs_version ) )
					len = sizeof( hs_version ) - 1;
				trap_FS_Read( hs_version, len, f );
				hs_version[len] = '\0';
				trap_FS_FCloseFile( f );
				while ( len > 0 && ( hs_version[len-1] == '\n' || hs_version[len-1] == '\r' || hs_version[len-1] == ' ' ) )
					hs_version[--len] = '\0';
			} else if ( f ) {
				trap_FS_FCloseFile( f );
			}
		...
		// Engine identity (from engine cvar)
		...
		// Hash auth token with nonce if token and user both exist
		hs_tokenHash[0] = '\0';
		hs_user[0] = '\0';
