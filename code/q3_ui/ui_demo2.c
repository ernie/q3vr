...
=================
*/
static char *BG_StripColor( char *string ) {
	char	*d;
	char	*s;
	int		c;
...
static void UI_DemosReadDir( void )
{
	...
	for ( i = 0; i < n; i++ ) {
		len = strlen( s );
		// don't add "." and ".." entries for demo root dir
	...
	if ( len > 2  && num_files < UI_MAX_DEMOS-1 ) {
		// count regular demo files across all known protocol versions
...
static int UI_cstrdiff( char * str ) {
	int diff;
	if ( !str )
		return 0;
	diff = 0;
...
static int UI_cstricmp( const char *src, const char *dst ) {
    int ret = 0;
    int c1 = 0, c2 = 0;
