...
static void AutospriteDeform( void ) {
	...
	// default, full view axes only for RF_VIEW_ORIENTED (worldEntity has
	// renderfx 0). The default basis is per quad, from the sightline to the
	// quad midpoint - a shared per-view basis would pitch and yaw with the
	// HMD, since view-derived axes rotate under pure head rotation while
	// sightlines to fixed points do not.
...
void RB_CalcColorFromEntity( unsigned char *dstColors )
{
	...
	c = backEnd.currentEntity->e.shaderRGBA.u32;
...
void RB_CalcColorFromOneMinusEntity( unsigned char *dstColors )
{
	...
	invModulate.rgba[0] = 255 - backEnd.currentEntity->e.shaderRGBA.rgba[0];
	invModulate.rgba[1] = 255 - backEnd.currentEntity->e.shaderRGBA.rgba[1];
	invModulate.rgba[2] = 255 - backEnd.currentEntity->e.shaderRGBA.rgba[2];
	invModulate.rgba[3] = 255 - backEnd.currentEntity->e.shaderRGBA.rgba[3];	// this trashes alpha, but the AGEN block fixes it
...
void RB_CalcAlphaFromEntity( unsigned char *dstColors )
{
	...
	{
		*dstColors = backEnd.currentEntity->e.shaderRGBA.rgba[3];
	}
}
...
void RB_CalcAlphaFromOneMinusEntity( unsigned char *dstColors )
{
	...
	for ( i = 0; i < tess.numVertexes; i++, dstColors += 4 )
	{
		*dstColors = 0xff - backEnd.currentEntity->e.shaderRGBA.rgba[3];
	}
}
...
static void RB_CalcDiffuseColor_scalar( unsigned char *colors )
{
	...
	// RF_OVERBRIGHT: apply overbright scaling to compensate for identityLight dimming.
	// Without this, colors are dimmed by identityLight in R_SetupEntityLighting
	// and the gamma pass overbright can't fully compensate due to clamping.
	if ( ent->e.renderfx & RF_OVERBRIGHT ) {
		overbright = (float)( 1 << tr.overbrightBits );
	} else {
		overbright = 1.0f;
	}
