...
(RB_SurfaceSpritePoly), so mirrors/portals see correctly-facing quads.
=====================
*/
...
void RE_RenderScene( const refdef_t *fd ) {
	...
	// When rendering 3D content for HUD mode 1 (in-world sprite),
	// use the HUD buffer dimensions instead of screen dimensions.
	// For HUD mode 2 (overlay) or virtual screen, use screen dimensions.
	if ( vr_currentHudDrawStatus->integer != 2 && tr.refdef.isHUD && tr.hudImage ) {
